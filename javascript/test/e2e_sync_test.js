...
async function requireDebugReveal(driver, iframeId) {
  await driver.switchTo().defaultContent();
  const iframe = await driver.findElement({ id: iframeId });
  await driver.switchTo().frame(iframe);
  ...
  if (!result) {
    throw new Error(
      'debugReveal() not available — set dom.oblivious.debug=true. ' +
      'Tests require debugReveal for content verification.'
    );
  }
}
...
async function run() {
  ...
  try {
    ...
    // Verify debugReveal is available (requires dom.oblivious.debug=true)
    await requireDebugReveal(driver, 'editorA');
    ...
    const allFiveBytesT6 = scanResults.every(r => r.error || r.byteLen === 5);
    console.log(`\n  All entries 5 bytes: ${allFiveBytesT6 ? 'YES' : 'NO — UNEXPECTED'}`);
